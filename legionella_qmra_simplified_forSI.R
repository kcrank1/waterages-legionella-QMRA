...
#Remove half of the scenario 2 water ages to limit it to one person living in a two person household

# Subset the dataframe to include only rows with Scenario 2
subset_df <- wa[wa$Scenario == 2, ]
# Create a sequence of row indices to keep every other row
...
mumax<-0.88/24 #per day convert to hours
Nmax<-10^5.94 #log10 copies/L convert to copies/L
...
tsh <- rtruncnorm(n, a = 0, mean = 7.8, sd = 0.02) # shower time, Exposure duration, truncated normal distribution, min
...
wat_conv <- B * tsh * (CVaer_conv * 1e3) * FD #CVaer_conv is in m3 so multiply by 1e3 to convert to L
...
###--Use growth model to calculate a concentration for each water age
wa<-wa%>%
  group_by(Scenario) %>%
  mutate(Conc = grow_baranyi(WaterAge, c(y0=C0, mumax=mumax, K=Nmax,h0=0))[,"y"]) #Concentration in GC/L
...
for (i in 1:nrow(wa)) {
  ...
  dose <- as.numeric(wa[i, "Conc"]) * wat_conv
  pinf <- P(kinf,gciu(dose))
  pcsi<-P(kcsi,gciu(dose))
  
  # Create a temporary dataframe with the results
  temp_df <- data.frame(
    Scenario = rep(as.numeric(wa[i, "Scenario"]), n),
    WaterAge = rep(as.numeric(wa[i, "WaterAge"]), n),
    # Time = rep(as.numeric(wa[i, "Time"]), n),
    #Conc = rep(as.numeric(wa[i, "Conc"]), n),
    # Dose = dose,
    Pinf = pinf, 
    Pcsi = pcsi,
    i=i
...
##-------Water ages vs Concentration, fig 2---
wa_sim<-runif(n,0,200)
df_wa_conc<- data.frame(
  wa_sim = wa_sim,
  conc = sapply(wa_sim, function(x) {
    y_values <- grow_baranyi(x, c(y0 = C0, mumax = mumax, K = Nmax, h0 = 0))[, "y"]
    gciu(y_values)
  })
)
...
#Starting concentrations set to 1 to 100 gc/L
C0_s<-runif(n,1,100)

#inf scenarios
pinf_sens1<-P(kinf,mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0=y, mumax=mumax, K=Nmax, h0=0))[, 2])))
}, waterage_1, C0_s)*B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) +(Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) +(Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) +(F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))

cor(C0_s,pinf_sens1,method="spearman")
pinf_sens2 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_2, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))

cor(C0_s,pinf_sens2,method="spearman")

pinf_sens3 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_3, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))


pinf_sens4 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_4, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))


pinf_sens5 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_5, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))


pinf_sens6 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_6, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))


pinf_sens7 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_7, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))


pinf_sens8 <- P(kinf, mapply(function(x, y) {
  gciu((as.numeric(grow_baranyi(x, c(y0 = y, mumax = mumax, K = Nmax, h0 = 0))[, 2])))
}, waterage_8, C0_s) * B * tsh * (((Caer1 * Vaer1) + (Caer2 * Vaer2) + (Caer3 * Vaer3) + (Caer4 * Vaer4) + (Caer5 * Vaer5) + (Caer6 * Vaer6) + (Caer7 * Vaer7) + (Caer8 * Vaer8) + (Caer9 * Vaer9) + (Caer10 * Vaer10)) * 1e3) * ((F1 * D1) + (F2 * D2) + (F3 * D3) + (F4 * D4) + (F5 * D5) + (F6 * D6) + (F7 * D7) + (F8 * D8) + (F9 * D9) + (F10 * D10)))
...
#INFECTION
sens1<-data.frame(waterage_1,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens1)
sens2<-data.frame(waterage_2,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens2)
sens3<-data.frame(waterage_3,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens3)
sens4<-data.frame(waterage_4,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens4)
sens5<-data.frame(waterage_5,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens5)
sens6<-data.frame(waterage_6,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens6)
sens7<-data.frame(waterage_7,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens7)
sens8<-data.frame(waterage_8,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kinf,pinf_sens8)
...
sens1<-data.frame(waterage_1,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens1)
sens2<-data.frame(waterage_2,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens2)
sens3<-data.frame(waterage_3,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens3)
sens4<-data.frame(waterage_4,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens4)
sens5<-data.frame(waterage_5,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens5)
sens6<-data.frame(waterage_6,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens6)
sens7<-data.frame(waterage_7,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens7)
sens8<-data.frame(waterage_8,B,tsh,Caer1,Caer2,Caer3,Caer4,Caer5,Caer6,Caer7,Caer8,Caer9,Caer10,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10, C0_s,kcsi,pcsi_sens8)
